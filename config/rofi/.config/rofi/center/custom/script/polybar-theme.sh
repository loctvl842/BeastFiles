...
function change_theme() {
  ...
  # dunstify -i "$CENTER_CUSTOM_LOGO_DIR/polybar.png" "Polybar Theme: $theme_name" -t 2000
	# re-launch polybar
...
MENU="$(rofi -sep "|" \
        -dmenu -i -p 'System' \
        -theme $CENTER_CUSTOM_APPEARANCE_DIR/polybar-theme.rasi \
        <<< "$previous|"$prefix"Monokai|"$prefix"Monokai++|"$prefix"Vscode|"$prefix"Tokyo Night|"$prefix"Mac Color")"
  case "$MENU" in
			*"$previous") $CENTER_CUSTOM_SCRIPT_DIR/polybar.sh;;
